...
@Service
public class TypeOfLotService {

    @Autowired
    public TypeOfLotRepository typeOfLotRepository;

    public Page<TypeOfLot> getAll(int page, int size) {
        int pageIndex = Math.max(0, page - 1);
        Pageable pageable = PageRequest.of(pageIndex, size);
        return typeOfLotRepository.findAll(pageable);
    }
}
